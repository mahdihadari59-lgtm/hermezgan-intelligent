...
class ExpertRouter {
  ...
  register(intentType, instance, meta = {}) {
    ...
    this.experts.set(intentType, instance);
    this.sources.set(intentType, meta);
  }

  get(intentType) {
    return this.experts.get(intentType) || null;
  }

  async autoLoad({ pluginsDir = null, expertsDir = null, manifestMap = {} } = {}) {
    if (pluginsDir && fs.existsSync(pluginsDir)) {
      ...
      for (const dirent of subdirs) {
        ...
        try {
          ...
          const intents = Array.isArray(manifest.intents)
            ? manifest.intents
            : [manifest.intent || dirent.name.toLowerCase()];

          for (const intentType of intents) {
            this.register(intentType, instance, {
              plugin: manifest.name || dirent.name,
