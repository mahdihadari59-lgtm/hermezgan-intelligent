...
function createRoutes(engines) {
  ...
  async function runTranslationPipeline(text, sessionId) {
    ...
    if (ragResult.found) {
      ...
      return { success: true, translation: ragResult.response, intent: intentResult.intent, dialect: dialectResult.dialect, confidence: ragResult.confidence, source: 'rag' };
    }
    ...
    if (llmResult.used) {
      ...
      return { success: true, translation: llmResult.translation, intent: intentResult.intent, dialect: dialectResult.dialect, confidence: llmResult.confidence, source: 'llm' };
    }
    ...
    return { success: true, translation, original: text, intent: intentResult.intent, dialect: dialectResult.dialect, confidence, matchedWords: matched, totalWords: words.length, source: 'dictionary' };
  }

  // Multilingual
  router.post('/multilingual/translate', (req, res) => {
    const { word, source, target } = req.body || {};
    if (!word || !source || !target) return res.status(400).json({ error: 'word، source و target را وارد کنید' });
  ...
  router.post('/detect', (req, res) => {
    ...
    const dialectResult = dialect.detect(text);
    res.json({ dialect: dialectResult.dialect, dialectLabel: dialectResult.label, confidence: dialectResult.confidence, scores: dialectResult.scores, grammarFeatures: features });
  });
  ...
  router.post('/learn', (req, res) => {
    ...
    if (!word || !translation) return res.status(400).json({ error: 'کلمه و ترجمه را وارد کنید' });
  ...
  router.get('/dictionary/category/:key', (req, res) => {
    ...
    if (Object.keys(words).length === 0) return res.status(404).json({ error: 'دسته‌ای با این کلید پیدا نشد' });
  ...
  router.get('/stats', (req, res) => {
    res.json({ dictionary: dictionary.getStats(), multilingual: multilingual.getStats(), rag: rag.getStats(), context: context.getStats(), llmEnabled: llm.enabled, version: '4.2.0' });
  });
