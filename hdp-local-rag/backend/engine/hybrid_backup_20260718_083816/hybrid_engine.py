...
استفاده می‌کند (Composition به‌جای God-Class):
    ...
    - text_utils.normalize_persian   → نرمال‌سازی متن
...
class HybridEngine:
    ...
    def _materialize(self, ranked: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            cur.execute(
                f"SELECT id, title, content, node_type, domain FROM graph_nodes WHERE id IN ({placeholders})",
                ids,
            )
