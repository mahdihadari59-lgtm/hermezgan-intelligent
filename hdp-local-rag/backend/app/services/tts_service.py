...
class ElevenLabsTTSProvider(TTSProvider):
    ...
    async def synthesize(self, text: str, language: str = "fa", **kwargs) -> Dict[str, Any]:
        try:
            if not self.api_key:
                return {"success": False, "error": "ELEVENLABS_API_KEY not configured", "provider": "elevenlabs"}
            if not self.voice_id:
                return {"success": False, "error": "ELEVENLABS_VOICE_ID not configured", "provider": "elevenlabs"}
            ...
            logger.exception("ElevenLabs TTS synthesis error")
