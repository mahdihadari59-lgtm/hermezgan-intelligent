...
function createRoutes(engines) {
  const { dictionary, multilingual, grammarRef, rag, grammar, intent, dialect, morphology, context, llm } = engines;
  ...
  router.get('/grammar/pronouns', (req, res) => {
    res.json({ tables: grammarRef.getPronouns() });
  });

  router.get('/grammar/verbs', (req, res) => {
    const verb = req.query.verb;
    res.json({ conjugations: grammarRef.getVerbConjugations(verb) });
  });

  router.get('/grammar/family', (req, res) => {
    res.json({ tables: grammarRef.getFamilyTerms() });
  });

  router.get('/grammar/idioms', (req, res) => {
    const category = req.query.category;
    res.json({ sections: grammarRef.getIdioms(category) });
  });

  router.get('/grammar/idioms/search', (req, res) => {
    const q = req.query.q;
    if (!q) return res.status(400).json({ error: 'پارامتر q را وارد کنید' });
    res.json({ results: grammarRef.searchIdiom(q) });
  });

  router.get('/grammar/dialogues', (req, res) => {
    const scenario = req.query.scenario;
    res.json({ scenarios: grammarRef.getDialogues(scenario) });
  });

  router.get('/grammar/object-marking', (req, res) => {
    res.json(grammarRef.getObjectMarking());
  });

  router.get('/grammar/rules', (req, res) => {
    res.json(grammarRef.getGrammarRules());
  });
  ...
  router.get('/stats', (req, res) => {
    res.json({
      ...
      grammarRef: grammarRef.getStats(),
      rag: rag.getStats(),
